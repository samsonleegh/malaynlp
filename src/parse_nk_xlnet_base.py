...
def get_bert(bert_model, bert_do_lower_case):
    ...
    # for polyaxon
    # model = XLNetModel.from_pretrained(
    #     '/polyaxon-data/aiap6/workspace/samson_lee/malaynlp/pretrained/')
    # tokenizer = XLNetTokenizer.from_pretrained(
    #     '/polyaxon-data/aiap6/workspace/samson_lee/malaynlp/pretrained/', do_lower_case=False
        ...
        for i in range(num_layers):
            ...
            if d_positional is None:
                ff = PositionwiseFeedForward(
                    d_model, d_ff, relu_dropout=relu_dropout, residual_dropout=residual_dropout)
            else:
                ff = PartitionedPositionwiseFeedForward(
                    d_model, d_ff, d_positional, relu_dropout=relu_dropout, residual_dropout=residual_dropout)
    ...
    def forward(self, xs, batch_idxs, extra_content_annotations=None):
        ...
        for i, (attn, ff) in enumerate(self.stacks):
            if i >= self.num_layers_position_only:
                ...
                res, current_attns = attn(
                    res, batch_idxs, qk_inp=timing_signal)
...
class NKChartParser(nn.Module):
    ...
    def __init__(
        ...
        if hparams.use_chars_lstm:
            ...
            self.project_elmo = nn.Linear(
                d_elmo_annotations, self.d_content, bias=False)
        elif hparams.use_bert or hparams.use_bert_only:
            logger.info(f"Get pretrained {hparams.bert_model} ...")
    ...
    def from_spec(cls, spec, model):
        ...
        if 'use_chars_concat' in hparams and hparams['use_chars_concat']:
            raise NotImplementedError(
                "Support for use_chars_concat has been removed")
    ...
    def split_batch(self, sentences, golds, subbatch_max_tokens=3000):
        if self.bert is not None:
            lens = [
                len(self.bert_tokenizer.tokenize(
                    ' '.join([word for (_, word) in sentence]))) + 2
    ...
    def parse_batch(self, sentences, golds=None, return_label_scores_charts=False):
        ...
        if self.char_encoder is not None:
            ...
            features = self.bert(
                all_input_ids, attention_mask=all_input_mask)[0]
        ...
        if not is_train:
            ...
            for i, (start, end) in enumerate(zip(fp_startpoints, fp_endpoints)):
                ...
                if self.f_tag is not None:
                    sentence = list(
                        zip(per_sentence_tags[i], [x[1] for x in sentence]))
        ...
        cells_scores = torch.gather(
            cells_label_scores, 1, cells_label[:, None])
        loss = cells_scores[:num_p].sum(
        ) - cells_scores[num_p:].sum() + paugment_total
