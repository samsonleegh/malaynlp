...
import tokens
# tokens = parse_nk # doesnt work, create tokens.py file
...
def run_train(args, hparams):
    if args.numpy_seed is not None:
        logging.info("Setting numpy random seed to {}...".format(args.numpy_seed))
    ...
    logger.info("Manual seed for pytorch:", seed_from_numpy)
    ...
    load_path =  None #'models/xlnet_dev=79.04.pt'
    if load_path is not None:
        print(f"Loading parameters from {load_path}")
        info = torch_load(load_path)
        parser = parse_nk.NKChartParser.from_spec(info['spec'], info['state_dict'])
    else:
        # logger.info("Loading Model from NKChartParser...")
        # model_path_present = os.path.exists('/polyaxon-data/aiap6/workspace/samson_lee/malaynlp/pretrained/')
        # logger.info(f"'/polyaxon-data/aiap6/workspace/samson_lee/malaynlp/pretrained/' {model_path_present}")
    ...
    def check_dev():
        ...
        if dev_fscore.fscore > best_dev_fscore:
            ...
            #     best_dev_model_path = os.path.join(output_path, 'xlnet')
            best_dev_model_path = "{}_dev={:.2f}".format(
                args.model_path_base, dev_fscore.fscore)
...
def main():
    ...
    # parser.add_argument("--model-path-base", default="models/xlnet")
    # parser.add_argument("--evalb-dir", default="EVALB/")
    # parser.add_argument("--train-path", default="data/train-aug.txt")
    # parser.add_argument("--dev-path", default="data/val-aug.txt")
    ...
    subparser.add_argument("--model-path-base", default="models/xlnet")
    subparser.add_argument("--evalb-dir", default="EVALB/")
    subparser.add_argument("--train-path", default="data/train-aug.txt")
    subparser.add_argument("--dev-path", default="data/val-aug.txt")
    subparser.add_argument("--batch-size", type=int, default=32)
    subparser.add_argument("--subbatch-max-tokens", type=int, default=500)
    subparser.add_argument("--eval-batch-size", type=int, default=16)
    ...
    subparser.add_argument("--evalb-dir", default="EVALB/")
    subparser.add_argument("--test-path", default="data/test-aug.txt")
    ...
    subparser.add_argument("--model-path-base", required=True)
    subparser.add_argument("--evalb-dir", default="EVALB/")
    subparser.add_argument("--viz-path", default="data/test-aug.txt")
    subparser.add_argument("--eval-batch-size", type=int, default=100)
